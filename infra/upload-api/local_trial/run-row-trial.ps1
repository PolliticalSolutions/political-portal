...
$reportName = "row-eligibility-trial-$timestamp.json"
...
$imageName = "political-portal-ocr-row-trial:local"
...
Write-Host "Building the isolated local OCR row-classification image..."
& $docker.Source build `
...
Write-Host (
    "Running $($pdfs.Count) PDF file(s) twice locally " +
    "(current production logic, then row-classification candidate)..."
)
$runArguments = @(
    "run", "--rm",
    "--network", "none",
...
if ($LASTEXITCODE -ne 0) {
    throw "The local OCR row-classification comparison did not complete."
}
