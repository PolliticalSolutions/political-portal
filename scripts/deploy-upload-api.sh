...
TESSERACT_LAYER_ARN="${TESSERACT_LAYER_ARN:-}"
...
if [ -n "$TESSERACT_LAYER_ARN" ]; then
  PARAM_OVERRIDES="$PARAM_OVERRIDES TesseractLayerArn=${TESSERACT_LAYER_ARN}"
fi
if [ -n "$SES_RECIPIENT_EMAIL" ]; then
  PARAM_OVERRIDES="$PARAM_OVERRIDES SesRecipientEmail=${SES_RECIPIENT_EMAIL}"
fi
