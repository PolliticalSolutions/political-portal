...
- **Council composition live (May 2026):** Migration `20260512000000_add_council_composition_columns.sql` applied — `council_data` now has `local_authority_id`, `controlling_party`, `control_type`, `total_seats`, `composition`, `composition_source`, `composition_verified_at`. 314 English councils populated from Open Council Data UK via `import_council_composition.py`. `LocalGovDetail.jsx` composition tab renders live seat-breakdown data. Weekly refresh runs every Monday at 07:00 UTC via `.github/workflows/sync-council-composition.yml` (requires `SUPABASE_SERVICE_KEY` repository secret). Manual on-demand runs available via `/sync-council-composition` Claude Code command or `workflow_dispatch` in GitHub Actions.
