...
| `EnquirePage.jsx` | `/enquire` | Contact/enquiry form; inserts into Supabase `enquiries` table |
| `BlogIndexPage.jsx` | `/blog` | Lists all blog posts from `content/blog/` markdown files |
| `BlogPostPage.jsx` | `/blog/:slug` | Individual blog post rendered from markdown |
| `Services.jsx` | `/services` | Services overview page |
...
| `Badge.jsx` | Small inline status badge with tone variants |
| `Button.jsx` | Polymorphic button (`as` prop); variants: primary, secondary, ghost |
...
- **Pure CSS only** — no Tailwind, no CSS-in-JS, no inline style objects except for dynamic values (colours, widths). All layout is in `src/index.css`.
